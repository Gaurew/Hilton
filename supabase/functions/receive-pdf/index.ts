...
Deno.serve(async (request) => {
  ...
  const { error: uploadError } = await client.storage.from("hilton-event-assets").upload(storagePath, file, { contentType: "application/pdf", upsert: false }); if (uploadError) return json({ error: "Unable to store PDF." }, 500);
  const { data, error } = await client.from("conversation_assets").insert({ conversation_id: conversationId, storage_path: storagePath, file_name: file.name, content_type: file.type }).select("id").single(); if (error) return json({ error: "Unable to record PDF." }, 500);
