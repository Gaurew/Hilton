...
Deno.serve(async (request) => {
  ...
  if (typeof body?.visitor_id !== "string" || typeof body?.visitor_access_key !== "string" || typeof body?.conversation_id !== "string") return respond({ error: "Workspace credentials are required." }, 400);
  ...
  const { data: messages, error: messageError } = await client.from("conversation_messages").select("id, role, category, content_markdown, created_at").eq("conversation_id", body.conversation_id).in("role", ["visitor", "agent"]).order("created_at");
  if (messageError) return respond({ error: "Unable to retrieve conversation updates." }, 500);
  const { data: assetRows, error: assetError } = await client.from("conversation_assets").select("id, file_name, content_type, storage_path, created_at").eq("conversation_id", body.conversation_id).order("created_at");
  if (assetError) return respond({ error: "Unable to retrieve generated documents." }, 500);
  ...
  return respond({ messages: messages ?? [], assets });
});
