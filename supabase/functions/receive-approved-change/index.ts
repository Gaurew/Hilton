...
Deno.serve(async (request) => {
  ...
  try { const message = await persistAgentMessage(body.conversation_id, "approved_change", body.markdown); return json({ message_id: message.id, status: "stored" }, 201); } catch { return json({ error: "Unable to store approved change." }, 500); }
});
