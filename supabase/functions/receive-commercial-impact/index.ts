...
Deno.serve(async (request) => {
  ...
  try { const message = await persistAgentMessage(body.conversation_id, "commercial_impact", body.markdown); return json({ message_id: message.id, status: "stored" }, 201); } catch (error) { console.error("Commercial impact persistence failed", error); return json({ error: "Unable to store commercial impact." }, 500); }
});
